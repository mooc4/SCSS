<template>
    <div>
        <div class="card" style=" margin-bottom: 5px ;">
            <el-input style="width: 240px;margin-right: 10px;" v-model="data.subject" placeholder="请输入课程名称"
        ...
        <div class="card" style="margin-top: 10px;">
            <el-table :data="data.tables" stripe border @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="teacher" label="教师名" width="180" />
...
const del = (id) => {
    ElMessageBox.confirm('确认要删除吗', '删除确认', { type: 'warning' }).then(() => {
        request.delete('/course/delete?id=' + id).then(res => {
            if (res.code === 1) {            // === 严格等于包括数据类型    == 会先转型在比较
