    ...
    <div>
        ...
        <div class="card" style="margin-top: 10px;">
            <el-table :data="data.tables" stripe border @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="name" label="学生名" width="180" />
                <el-table-column prop="subject" label="课程名" width="180" />
        ...
        <el-dialog v-model="data.formVisible" title="请输入修改数据" width="500">
            <el-form :model="data.form" label-width="80px" style="padding-right: 50px;padding-top: 20px;">
                ...
                <el-form-item label="成绩">
                    <el-input-number style="width: 180px ;" :min="0" v-model="data.form.grade" autocomplete="off"
                        placeholder="请输入成绩" />
                </el-form-item>
...
const add = () => {
    request.post('/sc/add', data.form).then(res => {
...
const update = () => {
    request.put('/sc/update', data.form).then(res => {
        if (res.code === 1) {            // === 严格等于包括数据类型    == 会先转型在比较
            ElMessage.success('保存成功')
            load()   //刷新表格
            data.formVisible = false
...
const del = (id) => {
    ElMessageBox.confirm('确认要删除吗', '删除确认', { type: 'warning' }).then(() => {
        request.delete('/sc/delete?id=' + id).then(res => {
            if (res.code === 1) {            // === 严格等于包括数据类型    == 会先转型在比较
...
const delBatch = () => {
    ...
    ElMessageBox.confirm('确认要删除吗', '删除确认', { type: 'warning' }).then(() => {
        request.delete('/sc/deleteBatch', { data: data.ids }).then(res => {
            if (res.code === 1) {            // === 严格等于包括数据类型    == 会先转型在比较
                ElMessage.success('删除成功')
                load()   //刷新表格
                data.formVisible = false
